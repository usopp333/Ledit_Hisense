...
void grating_link(LFile file,LCell cell)
{
	double WG_width_gr_link,MMI_length,MMI_width,taper_width,taper_length,radius,space_pro;
	double space_link,taper_length_grating,taper_width_grating,period,FF,grating_width;
	double pi,center_x,center_y,center_x0,center_y0;
	double factor;
	factor = 1000;
	pi = 3.1415926;
	
	WG_width_gr_link = 0.41*factor;//波导宽度
	...
	grating_left_link( file, cell, center_x, center_y, space_pro, WG_width_gr_link, taper_length_grating, 
	                   taper_width_grating, grating_width, period, FF );	
	
	
}

void grating_left_link( LFile file, LCell cell,  double x, double y, double space_pro, double WG_width_gr_link, 
						...
						double FF)
{
        ...
        Points [2].x = taper_x;
	    Points [2].y = taper_y-WG_width_gr_link/2;
	    Points [3].x = taper_x;
	    Points [3].y = taper_y+WG_width_gr_link/2;
		...
		Points [0].x = taper_x;
	    Points [0].y = taper_y+WG_width_gr_link/2;
	    Points [1].x = taper_x;
	    Points [1].y = taper_y-WG_width_gr_link/2;
        Points [2].x = taper_x+space_front;
	    Points [2].y = taper_y-PD_wg_width/2;
	    Points [3].x = taper_x+space_front;
	    Points [3].y = taper_y+PD_wg_width/2;
