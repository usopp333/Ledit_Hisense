...
//	main()
	...
	/* Parameter Setting */
		...
		// device_center_y=0.0E0*Scalefactor;	
		 ...
		 device_center_y=0.0E0*Scalefactor;	// 设置原点为器件的中心
	...
	/*	 device_L = 1000.0E0*Scalefactor;    */
		 ...
		 WG_connect_L2=100.0E0*Scalefactor; 		 
         //WG_connect_L3=350.0E0*Scalefactor;              
		 WG_connect_L3=200.0E0*Scalefactor; 
		...
		//WG_vertical_L= 300*Scalefactor;    // edit by LuZequan    
   ...
   /* positive MMI*/
    ...
    /* input grating and taper */
        ...
        /*left big  taper */
			...
			taper_start_x=device_center_x-winding_R-WG_short_L-taper_L-MMI_L-taper_L-WG_short_L-WG_connect_L1;;
			taper_start_y=device_center_y-winding_R-MMI_W/4+winding_R+WG_connect_L2+winding_R+WG_W/2;
			taper_stop_x=taper_start_x+big_taper_pic_L;
			taper_stop_y=taper_start_y-WG_W;
           // big_taper_pic();
		   big_taper_pic(
			   		file,
							cell,
        ...
        /* left big waveguide  (和taper链接的部分)  */
			...
			WG_start_x=taper_stop_x;
			WG_start_y=taper_stop_y+WG_W/2+big_taper_pic_W/2;
			//big_Waveguide();
			big_Waveguide(
						file,
							cell,
	...
	//	   LFile        File_Draw   = LCell_GetFile ( Cell_Draw );
		   ...
		   Points [0].x=WG_start_x+big_WG_L+(i*grating_P)/2;
		   Points [0].y=WG_start_y+big_WG_W;
		   Points [1].x=Points [0].x+grating_P/2;
  ...
  /* Start of input_taper*/
   ...
   /* start of left big_taper_pic*/
	...
	//	LFile        File_Draw   = LCell_GetFile ( Cell_Draw );
		...
		Points [0].y = taper_start_y;
		Points [1].x = Points [0].x;
		Points [1].y = taper_start_y-WG_W;
		Points [2].x = taper_stop_x;
		Points [2].y = taper_stop_y+WG_W/2-big_taper_pic_W/2;	
		Points [3].x = taper_stop_x;
		Points [3].y = Points [2].y+big_taper_pic_W; 
		LPolygon_New ( Cell_Draw, LLayer_Find( File_Draw, "Rib130" ), Points, 4);
	    }
   ...
   /* start of big_Waveguide */
							...
							)
	{
		...
		Points [0].x = WG_start_x;
		Points [0].y = WG_start_y;
		Points [1].x = WG_start_x;
		Points [1].y = Points [0].y-big_WG_W;
		Points [2].x = Points [1].x+big_WG_L;
		Points [2].y = Points [1].y;	
		Points [3].x = Points [0].x;
		Points [3].y = Points [2].y;
		LPolygon_New ( Cell_Draw, LLayer_Find( File_Draw, "Rib130" ), Points, 4);
